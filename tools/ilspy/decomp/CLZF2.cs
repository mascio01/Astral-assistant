...
public static class CLZF2
{
	...
	public static bool FakeDecompressFail = false;
	...
	public static byte[] Decompress(byte[] inputBytes)
	{
		int num = inputBytes.Length * 2;
		byte[] output = new byte[num];
		int num2 = lzf_decompress(inputBytes, ref output);
		while (true)
		{
			switch (num2)
			{
			...
			default:
				...
				goto case -1;
			case -1:
				FakeDecompressFail = false;
				Debug.LogWarning("Decompress failed (" + inputBytes.Length + " bytes)");
				return null;
			}
			num *= 2;
			output = new byte[num];
			num2 = lzf_decompress(inputBytes, ref output);
		}
	}
