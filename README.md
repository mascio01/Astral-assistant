...
| **`budget_guard.py`** | Reads the account credit once per process and returns a (credit, budget) pair consumed by the router. It is a **cost estimate / advisory signal**, not a hard spending cap: it does not by itself block calls. |
...
| File | Role |
|---|---|
| **`core_io.py`** | Shared runtime layer: console/rich setup, logging, error recovery and project scanning. (Atomic writes and JSON helpers live in the modules that need them.) |
...
| **`knowledge_map.json` / `knowledge_map.md`** | Knowledge index (API prices, benchmarks, routing history) generated by `selfmap.refresh_knowledge_map()`. The JSON is the source of truth; the Markdown is a generated view. Original source files remain authoritative. |
...
4. **Install dependencies**
   ...
   `requirements.txt` lists the base dependencies (`prompt_toolkit`, `rich`, `python-dotenv`, `openai`) plus optional ones for web scraping (`httpx`, `trafilatura`, `curl_cffi`) and local audio (`numpy`, `sounddevice`). The LLM client is created lazily: without `openai` or an API key, the independent modules (tools, memory, selfmap) still work.
